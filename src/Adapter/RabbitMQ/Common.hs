...
withState :: String -> Integer -> QueueAction a -> IO a
...
initConsumer (State _ conChan) queueName handler = do
  void . consumeMsgs conChan queueName Ack $ \(msg, env) ->
    void . forkIO $ do
      result <- handler msg
      if result
        then ackEnv env
  ...
  case eitherDecode' (msgBody msg) of
    Left err ->
      withMsgAndErr msg err $ do
        ...
        return False
    Right payload -> do
      result <- CS.tryAny (handler payload)
      case result of
        Left err ->
          withMsgAndErr msg (displayException err) $ do
            $(logTM)
              ErrorS
              "There was an exception processing the msg. Rejecting."
            return False
